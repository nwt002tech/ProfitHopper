...
def get_casinos_full(active_only: bool=True) -> pd.DataFrame:
    c=_client(); empty=pd.DataFrame(columns=["id","name","city","state","latitude","longitude","is_active","inserted_at","updated_at"])
...
def _ensure_game_cols(df: pd.DataFrame) -> pd.DataFrame:
    df=_safe_copy_df(df)
    expected=["id","name","type","game_type","rtp","volatility","bonus_frequency","min_bet",
              "advantage_play_potential","is_hidden","is_unavailable","image_url","source_url",
              "tips","updated_at","score"]
    for col in expected:
        if col not in df.columns:
...
def load_game_data(active_only: bool=True) -> pd.DataFrame:
    c=_client()
    if c is None: return _ensure_game_cols(pd.DataFrame())
    try:
        ...
        # enforce A→Z by name for UI consistency
