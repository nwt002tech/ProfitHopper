...
def _client(with_service: bool = False):
    """Anon by default; service role for writes or privileged reads."""
...
def get_casinos() -> List[str]:
    """(Back‑compat) Return list of active casino names."""
    df = get_casinos_full(active_only=True)
    return df["name"].dropna().astype(str).tolist()

def update_casino_coords(casino_id: str, lat: float, lon: float) -> bool:
    """Write latitude/longitude for a casino (service role key required)."""
...
# ----------------------------
# Games API (this adds load_game_data expected by app.py)
# ----------------------------
_GAMES_COLS_ORDER = [
...
def _norm_games_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in _GAMES_COLS_ORDER:
        if c not in df.columns:
            df[c] = None
    # types
    bool_cols = ["is_hidden","is_unavailable"]
    for b in bool_cols:
        if b in df.columns:
            df[b] = df[b].fillna(False).astype(bool)
    num_cols_float = ["rtp","bonus_frequency","min_bet","score"]
    for n in num_cols_float:
        if n in df.columns:
            df[n] = pd.to_numeric(df[n], errors="coerce")
    num_cols_int = ["volatility","advantage_play_potential"]
    for n in num_cols_int:
        if n in df.columns:
...
def load_game_data(
    ...
    """
    Load games from public.games. Optionally merge per‑casino availability and filter.

    Args:
      casino: if provided and include_availability=True, merges an
              'is_unavailable_at_casino' boolean for this casino.
      include_availability: join game_availability when True.
      limit: max rows to return.
      only_visible: if True, filter out rows where games.is_hidden is True.
      additional_filters: dict of column -> value to filter on (e.g., {"game_type":"Slots"})

    Returns:
      pd.DataFrame with games. If casino provided, includes 'is_unavailable_at_casino'.
    """
    c = _client()
    if not c:
        return pd.DataFrame(columns=_GAMES_COLS_ORDER)
    ...
    # Merge per‑casino availability if requested
    if include_availability and casino:
        try:
            q2 = c.table("game_availability").select("game_id, casino, is_unavailable")
            q2 = q2.ilike("casino", str(casino).strip())
            res2 = q2.execute()
            avail = pd.DataFrame(res2.data or [])
            if not avail.empty:
                avail = avail[["game_id","is_unavailable"]].rename(
                    columns={"is_unavailable": "is_unavailable_at_casino"}
                )
            ...
            # If the join fails for any reason, keep going without the extra column
            games["is_unavailable_at_casino"] = False
...
# ----------------------------
# Convenience: small fetchers some older code expects
# ----------------------------
