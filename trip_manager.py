...
def get_current_trip_sessions():
    return [s for s in st.session_state.session_log if s['trip_id'] == st.session_state.current_trip_id]

def get_trip_profit():
    trip_sessions = get_current_trip_sessions()
    return sum(s['profit'] for s in trip_sessions)

def get_current_bankroll():
    return st.session_state.trip_settings['starting_bankroll'] + get_trip_profit()

def get_session_bankroll():
    return st.session_state.trip_settings['starting_bankroll'] / st.session_state.trip_settings['num_sessions']