...
def render_sidebar():
    """Render the sidebar with trip settings"""
    with st.sidebar:
        ...
        <div style="display:flex; align-items:center; margin-bottom:20px;">
            <span style="font-weight:bold; margin-right:10px; color:white;">Current Trip ID:</span>
            <span class="trip-id-badge">{st.session_state.current_trip_id}</span>
        </div>
        """, unsafe_allow_html=True)
        
        # Casino selection
        new_casino = st.text_input("Add New Casino", key="new_casino")
        ...
        # Progress bar
        session_progress = len(trip_sessions) / st.session_state.trip_settings['num_sessions']
